...
class AristaCommunication(Communication):
    ...
    def add_acl(self, acl_type: ACL, interface):
        if acl_type is ACL.DEST_64:
            self.__interface_cmd('ipv6 access-group dest64 in', interface)
        elif acl_type is ACL.DEST_48:
            self.__interface_cmd('ipv6 access-group dest48 in', interface)
        elif acl_type is ACL.SOURCE_64:
            self.__interface_cmd('ipv6 access-group src64 in', interface)

    def remove_acl(self, acl_type: ACL, interface):
        if acl_type is ACL.DEST_64:
            self.__interface_cmd('no ipv6 access-group dest64 in', interface)
        elif acl_type is ACL.DEST_48:
            self.__interface_cmd('no ipv6 access-group dest48 in', interface)
        elif acl_type is ACL.SOURCE_64:
            self.__interface_cmd('no ipv6 access-group src64 in', interface)
